...
import { ArrowDownLeft, ArrowUpRight, Eye, WalletCards } from 'lucide-react';
      ...
      <section className="rounded-b-[2.5rem] bg-gradient-to-br from-violet-950 via-violet-800 to-fuchsia-700 px-5 pb-8 pt-7 text-white md:rounded-none md:px-8">
        <div className="mx-auto max-w-6xl">
          ...
          <strong className="block text-4xl font-black tracking-tight">
            {showBalance ? money(data?.currentBalance ?? 0) : 'R$ •••••'}
          </strong>

          <div className="mt-6 grid grid-cols-2 gap-3 md:grid-cols-4">
            <div className="rounded-3xl bg-white/10 p-4 backdrop-blur">
              <p className="text-xs text-violet-100">Resultado</p>
              <strong className="mt-1 block text-lg">{money(data?.resultMonth ?? 0)}</strong>
            </div>
            <div className="rounded-3xl bg-white/10 p-4 backdrop-blur">
              <p className="text-xs text-violet-100">Contas</p>
              <strong className="mt-1 block text-lg">{data?.accounts.length ?? 0}</strong>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-6xl space-y-6 px-5 py-6 md:px-8">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          ...
          <StatCard
            ...
            value={money(data?.currentBalance ?? 0)}
          ...
          <div className="flex gap-3 overflow-x-auto pb-1">
            {data?.accounts.map((account) => (
              ...
              >
                ...
                <strong className="mt-2 block text-lg text-slate-950">{money(account.balance)}</strong>
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          <div className="mb-3 flex items-center justify-between">
            <h2 className="text-lg font-black text-slate-950">Últimas movimentações</h2>
          </div>
            ...
            {data?.lastTransactions.length ? (
              data.lastTransactions.map((transaction) => (
                <TransactionCard
                  ...
                  date={new Date(transaction.transactionDate).toLocaleDateString('pt-BR')}
